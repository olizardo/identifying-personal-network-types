...
In the bottom-left panel, the Centered Star configuration ($n = 143$) is dominated by a substantial spike in betweenness centralization ($+1.38$ SD)---the largest positive deviation observed for any metric outside of regular density. Crucially, this focal brokerage pattern operates within relatively compact networks: network size is distinctly below average ($-0.45$ SD; $\xbar = 21.6$ alters), while alter--alter density ($-0.06$ SD) and community modularity ($+0.01$ SD) hover near the sample mean, and diameter is modestly elevated ($+0.18$ SD).
