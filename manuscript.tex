...
\vspace{1ex}
\noindent \textbf{Keywords:} Personal networks, egocentric network analysis, structural typologies, classification trees, Markov models, social support, relational multiplexity.
\end{abstract}
...
\input{tables/table1_structural_metrics.tex}
...
\input{tables/table2_cluster_selection.tex}
...
\input{tables/table3_markov_transitions.tex}
...
\input{tables/table4_social_support.tex}
...
Taken together, the empirical findings and topological analyses presented across \Cref{tab:structural_metrics,tab:cluster_selection,tab:markov_transitions,tab:social_support} and \Cref{fig:size_distribution,fig:metric_correlations,fig:cluster_selection,fig:cluster_profiles,fig:network_archetypes,fig:decision_tree,fig:comparative_trees,fig:longitudinal_transitions,fig:social_support} provide a comprehensive structural cartography of egocentric relational environments in a residential collegiate setting. Rather than supporting a model of continuous, unstructured variation in personal network connectivity, the evidence demonstrates that interpersonal networks naturally coalesce into discrete, recurrent topological configurations that mirror and extend the foundational archetypes identified by \citet{bidart2018personal} and \citet{vacca2020structure}.

At the core of this typology are four distinct structural regimes: expansive and modular ``Pearl Collar Networks'' ($\xbar = 47.9$ alters), decentralized ``Segmented Networks'' ($\xbar = 27.4$ alters), centralized ``Centered Star Networks'' ($\xbar = 21.6$ alters), and tightly cohesive ``Regular Dense Networks'' ($\xbar = 12.6$ alters). Notably, we find no empirical evidence in this residential context for separate ``Centered Dense'' or ``Dispersed'' clusters, indicating that under conditions of daily campus co-presence, extreme density and high betweenness centralization rarely co-occur, while isolates and fragmented dyads are readily integrated into larger relational structures.

Our empirical decision tree moves beyond the subjective heuristics of prior work, classifying network typology membership with 92.9\% overall accuracy. The tree establishes portable, data-driven cutoffs, identifying alter--alter density ($\ge 0.55$) as the primary structural threshold separating cohesive social enclaves from internally differentiated configurations, followed by betweenness centralization ($0.24$), diameter ($\ge 3.5$), and network size ($\ge 37$). These explicit rules provide comparative researchers with objective criteria to classify personal network forms without relying on algorithmic black-box clustering.
...
Finally, our alter-level functional support analysis bridges the longstanding divide between structural and compositional network paradigms. While baseline companionship is pervasive across all typologies ($\sim 88\%$ to $93\%$), substantive functional aid (informational advice, emotional comfort, financial assistance), support multiplexity ($\ge 3$ functions), tie closeness, and interpersonal trust concentrate heavily in small, tightly knit Regular Dense enclaves. Expansive Pearl Collar networks maximize topological reach and bridge across modular student worlds, but do so by diluting the proportion of multiplex, emotionally intensive, and high-trust ties.
...
\subsection{Theoretical Implications: Toward a Structural Sociology of Personal Networks}
...
Most centrally, our results resolve the longstanding tension between deductive theoretical taxonomies and inductive algorithmic clustering \citep{bidart2018personal, gonzalezcasado2024towards, vacca2020structure}. For decades, researchers remained caught between transparent but arbitrary heuristic cutoffs derived from small, idiosyncratic samples \citep{bidart2018personal} and unsupervised clustering algorithms that partition networks without yielding portable, interpretable rules \citep{vacca2020structure}. By uniting unsupervised $k$-means clustering with recursive partitioning classification trees, our analytical workflow demonstrates that egocentric relational space can be mapped with mathematical rigor while producing explicit, transparent decision boundaries that other scholars can operationalize directly in comparative datasets.

Furthermore, our findings illuminate the dynamic, temporal nature of personal social capital. Classical network theory often treats personal networks as static structures or enduring individual attributes \citep{fischer1982dwell, mccarty2002structure}. Our longitudinal transition models challenge this static view, demonstrating that personal networks are developmental systems that oscillate between localized cohesive enclaves and expansive modular bridging pathways. Students do not inhabit permanent structural categories; rather, they transition across network regimes as institutional demands, residential reassignments, and developmental milestones reshape their social ecologies.

Finally, our findings reconcile the structural and compositional traditions in egocentric network analysis. Rather than viewing graph topology and relational content as competing paradigms \citep{antonucci2013convoy, mccarty2002structure, offer2018does}, our results show that they represent two sides of the same relational coin. Egocentric network geometry establishes the physical and communicative constraints that govern the bandwidth of social support: expansive, modular topologies maximize bridging social capital and access to novel information \citep{burt1992structural, granovetter1973strength}, but do so at the cost of relational multiplexity and emotional depth. Conversely, small, cohesive enclaves sacrifice structural reach in order to cultivate dense mutual trust, emotional solidarity, and multi-functional safety nets \citep{coleman1988social, lin2001social}. By integrating topological geometry, empirical decision rules, longitudinal transitions, and functional support provision, this study provides a unified structural foundation for understanding how personal networks organize interpersonal life.
